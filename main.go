...
// telling the program that the results ned to be in this format
...
// a list of all user agents
// user agents are the different type of browsers; Chrome, FireFox, Safari, and the various browser forks that use those JS engines
...
func randomUserAgent() string {
	...
	// from the length of user agents, select a random number
	randNum := rand.Int() % len(userAgents)
	// to access a particular value in a slice there needs to be an index passed into an array 
	// getting the index from randNum variable
	return userAgents[randNum]
}

// creating search queries for Google
// accepts search term, country code, pages, count
// returns either the slice of type string or an error
func buildGoogleUrls(searchTerm, countryCode, languageCode string, pages, count int)([]string, error) {
	toScrape := []string{}

	// refining the search term
	// trimming all the space in the search term
	searchTerm = strings.Trim(searchTerm, " ")
	// + is used in the URL instead of spaces; lookup the specifics of Google search queries
	searchTerm = strings.Replace(searchTerm, " ", "+", -1)

	// find the realvent Google domain (URL) for the country code
	// helps find the index for the countryCode
	// googleBase is the Google link
	if googleBase, found := googleDomains[countryCode]; found {
		// pages is the number of pages scrapped from Google
		// for the number of pages is how many times the for loop will iterate over itself
		for i := 0; i < pages; i++ {
			start := i * count
			// the URL that will be built
			...
			// appending scrapeURL to toScrape
...
// accepting res (response) as type http.Response and resultCounter (rank) as an int
// returns SearchResult and an error
func googleResultParsing(response *http.Response, rank int)([]SearchResult, error) {
	...
	for i := range sel.Nodes {
		...
		if link != "" && link != "#" && !strings.HasPrefix(link, "/") {
			// create one search result for the SearchResult struct
			result := SearchResult{
				rank,
				link,
				title,
			...
			// appending result var to the slice from results var 
...
func getScrapeClient(proxyString interface{}) *http.Client {
	...
	default:
		// returning default http client that's used to scrape
		return &http.Client{}
	}
}

// returning a slice of SearchResult and err
// returning err because in GoogleScrape it's either a result or error
// accepting searchTerm, countryCode, pages, count
func GoogleScrape(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count, backoff int)([]SearchResult, error) {
	...
	resultCounter := 0
	// creating search queries for Google
	// expecting a slice of type string
	...
	for _, page := range googlePages {
		// scrapeClientRequest will make the request to the query above (googlePages)
...
// returns either an http.Response or an error
func scrapeClientRequest(searchURL string, proxyString interface{})(*http.Response, error) {
	// if there's something in the proxy string it will create a baseClient, if not it will return a default baseClient
...
func main() {
	// GoogleScrape is where the text is posted
	// searchTerm, countryCode, languageCode, proxyString, pages, count, seconds each iteration of the for loop sleeps in GoogleScrape (backoff): tells the program to get 30 results
